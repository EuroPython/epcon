...
def after_install(options, home_dir):
    ...
    subprocess.call([pip, 'install', '-q', '--requirement', 'requirements.txt'])
...
'''
    ...
    for fname in os.listdir(rpath):
        if os.path.isdir(fname) and fname.endswith('_site'):
            print 'make wrapper for:', fname
            wpath = os.path.join(fname, 'manage.sh')
            f = file(wpath, 'w')
            f.write(MWRAPPER % (os.path.join(os.path.abspath(home_dir), 'bin', 'activate'), os.path.join(rpath, fname, 'manage.py')))
            f.close()
            os.chmod(wpath, 0755)
