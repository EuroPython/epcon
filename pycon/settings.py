...
# TODO: Clarification on the translation status required.
...
class Base(Configuration):
    ...
    def ALLOWED_HOSTS(self):
        return self.ALLOWED_HOSTS_FROM_ENV.split(',')
    ...
    @property
    def MEDIA_ROOT(self):
        return self.DATA_DIR + '/media_public'

    @property
    def SECURE_MEDIA_ROOT(self):
        return self.DATA_DIR + '/media_private'
    ...
    # Database config
    # TODO: Refactor this to use values.DatabaseURLValue as the next step
    # https://django-configurations.readthedocs.io/en/stable/values/#url-based-values
    DATABASE_TYPE = values.Value(environ_name='DATABASE')
    ...
    def DATABASES(self):
        if self.DATABASE_TYPE == "postgres":
            ...
            'default': {
                ...
                'NAME': self.SITE_DATA_ROOT + '/p3.db',
            }
        }
    ...
    SERVER_EMAIL = values.Value('noreply@europython.eu')

    # Email sender address used per default for emails to e.g. attendees
    DEFAULT_FROM_EMAIL = values.Value('info@europython.eu')
    ...
    SECRET_KEY = values.Value('your-secret-key')
    ...
    RECAPTCHA_PRIVATE_KEY = values.Value('no-private-key-set')
    # The key below is registered for EuroPython domains
    RECAPTCHA_PUBLIC_KEY = values.Value('6LdFmQcTAAAAAN1xx4M5UN6yg4TwFRXUwIrH5iGh')
    ...
    PAGE_LANGUAGE_MAPPING = pristinemethod(lambda lang: PAGE_LANGUAGES[0][0])
    ...
    #
    # TODO: Remove the comment or the feature below.
    ...
    @property
    def CONFERENCE_TALKS_RANKING_FILE(self):
        return self.SITE_DATA_ROOT + '/rankings.txt'

    @property
    def CONFERENCE_ADMIN_TICKETS_STATS_EMAIL_LOG(self):
        return self.SITE_DATA_ROOT + '/admin_ticket_emails.txt'
    ...
    # TODO: Move this out of Django settings
    ...
    def CONFERENCE_TICKETS(conf, ticket_type=None, fare_code=None):
        # TODO: Can this be moved out of Django settings?
    ...
    @property
    def CRONTAB_COMMAND_PREFIX(self):
        return 'DATA_DIR=%s OTHER_STUFF=%s' % (self.DATA_DIR, self.OTHER_STUFF)
    ...
    #
    STRIPE_ENABLED = True
    STRIPE_SECRET_KEY = values.Value()
    STRIPE_PUBLISHABLE_KEY = values.Value()
    STRIPE_COMPANY_NAME = values.Value()
    STRIPE_COMPANY_LOGO = values.Value()
    STRIPE_CURRENCY = "EUR"
    ...
    PAYPAL_RECEIVER_EMAIL = values.Value()

    # If the merchant account is a debug one set this flag to True
    PAYPAL_TEST = values.BooleanValue(False)
    ...
    def setup(cls):
        ...
        # TODO: There has to be a claner way of doing this
        # Required to import email_template
        sys.path.insert(0, os.path.join(cls.PROJECT_DIR, 'deps'))
    ...
    def post_setup(cls):
        ...
        check = os.path.commonprefix((settings.MEDIA_ROOT, settings.SECURE_MEDIA_ROOT))
        if check.startswith(settings.MEDIA_ROOT):
