...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orders = {}
        for x in orm.Invoice.objects.all():
            order_vats = orders.get(x.order_id, set())
            if not order_vats:
                order = x.order
                order_vats = set()
                for item in order.orderitem_set.all():
                    order_vats.add(item.vat_id)
                try:
                    vat_id = order_vats.pop()
                    if order_vats:
                        orders[order.id] = order_vats
                except:
                    vat_id = 1
            else:
                try:
