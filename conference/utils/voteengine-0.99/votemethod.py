...
def borda(o):
	...
	for i in range(n):
		...
		print(ljust(o.cand_l[i],4), rjust(repr(borda),9))
...
def bucklin(o):
	...
	while level>0:
		...
		for i in range(n):
			print(ljust(o.cand_l[i],4), rjust(repr(vts[i]),9))
		print()
		max=-1
		cand=0
...
def c_irv(o):
	...
	act=list(range(n)) # active / non-eliminated candidates
	while len(act)>1:
		vts=zeros((n),Int32)
		for x in o.ballot_tbl:
			b=x.ballot
...
def copeland(o):
	...
	print("Cand  Wins")
	for i in range(n):
		print(ljust(o.cand_l[i],4), rjust(repr(0.5*(sc[i]-1)),9))
...
def minmax(o):
	...
	print("Cand  Max Loss")
	for x in range(n):
		print(ljust(o.cand_l[x],4), rjust(repr(sc[x]),9))
	winmat=zeros((n,n),int_)
	for i in range(n):
		for j in range(n):
			if i!=j and sc[i]<sc[j]: winmat[i,j]=1
...
def pw_elim(o):
	...
	while len(act)>1:
		...
		print("Cand  Max Loss")
		for i in act:
			print(ljust(o.cand_l[i],4), rjust(repr(sc[i]),9))
...
def s_irv(o):
	n=len(o.cand_l)
	act=list(range(n)) # non-eliminated candidates
	print('Smith//IRV')
...
def rp(o):
	...
	for i in range(n): pw[i,i]=maxint
	path=zeros((n,n),Int32)
	for i in range(n): path[i,i]=maxint
...
def ukvt(o):
	...
	print('uk.* voting')
	if candSQ!=-1:
		for i in range(n): # note that candSQ is always removed from act
			if o.pw_tbl[i,candSQ]<12: 
				act.remove(i)
				continue

	if candSQ!=-1:
		print('Candidates that beat SQ by 12 or more: ', end=' ') 
		if act==[]:
			print("None.  SQ wins")
			return
		for i in act:
