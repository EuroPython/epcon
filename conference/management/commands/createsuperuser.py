...
Overriding the tool provided by django - the only available alternative would be
creating a `django.contrib.auth.user` post-create signal, but it would require additional
refactoring to e.g. prevent email sending. Sadly, there's no way to override
the default `django.contrib.auth.user` manager create_user method.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if username:
            ...
            # This part is different from the default django-admin command
            AssopyUser._default_manager.db_manager(database).create_superuser(**user_data)
