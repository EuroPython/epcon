...
def test_add_comments_anonymous(client):
    ...
    resp = client.post(reverse('hcomments-post-comment'), data={
        "comment": "some comment",
        "content_type": "{}.{}".format(talk_content_type.app_label, talk_content_type.model),
        "object_pk": talk.pk,
        "timestamp": time.time(),
    })

    assert redirects_to(resp, reverse('login'))
