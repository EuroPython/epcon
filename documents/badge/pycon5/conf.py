...
_FONT_INFO = 'Ubuntu-Title.ttf'
_FONT_NAME = 'Ubuntu-Title.ttf'
_FONTS = {
...
PY_LOGO = {
    'all': Image.open('star.png').convert('RGBA').resize((64, 64)),
}

def tickets(tickets):
    groups = {}
    for k in ('staff', 'standard'):
        groups[k] = {
...
def ticket(image, ticket, utils):
    ...
    if ticket['badge_image']:
        ...
        key = 'staff' if ticket.get('staff') else 'all'
        logo = PY_LOGO.get(key) or PY_LOGO['all']
    rows = [
        (first_name, (100, name_y[0]), font, color_name),
        (last_name, (100, name_y[1]), font, color_name),
        (tagline, (100, 790), _FONTS['info'], color_info),
    ] + [
        (logo, (100 + (logo.size[0] + 20) * ix, 855)) for ix in range(ticket.get('experience', 0))
    ]
    mirrored = [
